...
public class AdminService {
    ...
    public AdminListResponseDto getAdminList(AdminListRequestDto requestDto) {
        //검색 규칙 설정
        Pageable pageable = PageRequest.of(//아래 내용들을 모두 챙겨 하나로 묶어 new Pageable 객체를 생성하는것????????
        ...
        Page<Admin> adminPage = adminRepository.searchAdmins(//searchAdmins 만든 검색 규칙
                requestDto.getKeyword(), requestDto.getRole(), requestDto.getStatus(), pageable
        );

        //엔티티->DTO 변환
        List<AdminResponseDto> list = adminPage.getContent().stream()//.getContent() Admin 객체를 꺼내서 .stream()하나씩
