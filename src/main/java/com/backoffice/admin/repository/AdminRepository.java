...
public interface AdminRepository extends JpaRepository<Admin, Long> {
    ...
    //유연한사람->핵심 코어를 아는사람
    @Query("SELECT a FROM Admin a WHERE " +//Admin 객체(테이블) a를 모두 객체 a 전체 조회하겠다. 어떻게?(조건)
            "(:kw IS NULL OR a.name LIKE %:kw% OR a.email LIKE %:kw%) AND " +
            //외부(:) 변수(kw)[입력 검색어]에 값이 비어있나? or a테이블에 name(DB에 저장된 이름)이 있으면 LIKE(~같은지 봐라) kw앞뒤로 어떤내용이 와도 상관없고(%)
    ...
    Page<Admin> searchAdmins(@Param("kw") String kw,
    //한페이지 분량만 가져오는데 내용물은 <Admin>이다 + 메서드이름 +  @Param("kw"): 쿼리문 안에 적었던 :kw 자리와 이 변수 kw를 서로 연결(바인딩)하라는 명령
